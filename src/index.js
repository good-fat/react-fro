...
const create = () => {
  ...
  const mix = (func, other_name) => {
    ...
    pmfl.make2().add((condition_data) => {
      ...
      let [mixing, func, , temp_func] = match_data
      mixing[func.name] = temp_func
    }).neither((match_data) => {
      let [mixing, , other_name, temp_func] = match_data
  ...
  const mark = (str_data) => {
    ...
    temp_obj.mixing = (func, other_name) => {
      if (other_name === undefined)
        marked_data.get(str_data).push(["mixing", func.name])
      else {
  ...
  const mix_apply = (condition, ...args) => {
    if(condition !== undefined) {
      if(type.of(condition) === type.function) {
        if(!condition(fro.constant, virtual_state)) {
          return fro.mixing
